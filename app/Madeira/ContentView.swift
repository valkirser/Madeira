...
struct ContentView: View {
    ...
    private func runWineFullSequence(reattachAttempted: Bool = false) {
        ...
        // External drive: resolves the bookmark (if any) and opens
        // security-scoped access for this run BEFORE wineserver starts --
        // same ordering requirement as every other override file (ml588).
        // Released in RunStatus.end() once Wine actually exits.
        if let driveStatus = ExternalDriveStore.shared.prepareForLaunch() {
            logStore.log(driveStatus, level: ExternalDriveStore.shared.lastError == nil ? .success : .error)
        }
        RunStatus.shared.begin(preparing: true)
