...
final class RunStatus: ObservableObject {
    ...
    private func end() {
        ...
        sawWine = false
        phase = .idle
        // Pairs with ExternalDriveStore.prepareForLaunch(): the run that just
        // ended is the last thing that could have touched the external
        // drive, so release the security-scoped access it opened.
        ExternalDriveStore.shared.finishAfterRun()
