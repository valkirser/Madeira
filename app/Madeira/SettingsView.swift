...
struct SettingsView: View {
    ...
    @ObservedObject private var driveStore = ExternalDriveStore.shared
    ...
    private var externalDriveSection: some View {
        Section {
            LabeledContent("Status", value: driveStore.displayName ?? "Not connected")
            if let error = driveStore.lastError {
                Text(error)
                    .font(.caption)
                    .foregroundStyle(AppTheme.warn)
            }
            Button(driveStore.isConnected ? "Choose a Different Folder…" : "Choose Folder…") {
            ...
            Text("Maps a folder from Files — an external drive, SD card reader, network "
                + "share, or cloud location — to \(ExternalDriveStore.driveLetter) inside "
                + "Wine. Access is re-requested from the system on every launch, so "
                + "reconnect here if the drive was unplugged or the folder moved.")
        }
